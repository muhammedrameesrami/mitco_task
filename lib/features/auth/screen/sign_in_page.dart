...
import '../../../core/common_bloc/order_model_bloc.dart';
...
class _SignInPageState extends State<SignInPage> {
  ...
  checkId() async {
    ...
    if(
preferences.containsKey('orderId')
    ){
     final res= preferences.getString('orderId');
     final myUserId=await FirebaseFirestore.instance.collection('order').doc(res).get();
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: TextFormField(controller: emil,
                ...
                Align(
                    ...
                    child: Text(
                      'forgot password?',
                      style:
                      TextStyle(color: Colors.white30, fontSize: w * 0.03),
                    )),
                SizedBox(
                  height: h * 0.05,
                ),
...
showSnackBar(message: state.message, context: context);
                  ...
                  builder: (context, state) {
                    ...
                    return InkWell(onTap: () {
                      ...
                      if(password.text.trim().isNotEmpty||emil.text.trim().isNotEmpty)
                    context.read<AuthScreenBloc>().add(AuthSignIn(password:password.text.trim(), email: emil.text.trim(), context: context));
                    },
                      child: Container(
                        height: h * 0.07,
                        width: w,
                ...
                Align(alignment: Alignment.center,
                  child: Text('Dont have an account?',
                      style:
                      TextStyle(color: Colors.white60, fontSize: w * 0.04)),)
                , SizedBox(height: h * 0.03,),
                Align(alignment: Alignment.center,
                  child: Text('SignUp',
                      style:
                      TextStyle(color: Colors.blueAccent, fontSize: w * 0.05)),
                )
              ],
            ),
          ),
